...
public final class DiffWebViewCoordinator: NSObject {
  ...
  /// Callback when the rendered content height changes. Useful for sizing the
  ...
  func cleanup() {
    ...
    onAnnotationDraftSubmit = nil
    onAnnotationDraftCancel = nil
  ...
  private func handleMessage(_ message: DiffWebViewEvent) {
    ...
    case .annotationDraftSubmitted(let annotationID, let commentID, let body, let side, let lineNumber):
      onAnnotationDraftSubmit?(annotationID, commentID, body, side, lineNumber)

    case .annotationDraftCancelled(let annotationID, let commentID, let side, let lineNumber):
      onAnnotationDraftCancel?(annotationID, commentID, side, lineNumber)
...
extension DiffWebViewCoordinator: WKScriptMessageHandler {
  ...
  public func userContentController(
    ...
    case "annotationDraftSubmitted":
      let annotationID = body["annotationID"] as? String ?? ""
      let commentID = body["commentID"] as? String ?? ""
      let draftBody = body["body"] as? String ?? ""
      let side = body["side"] as? String ?? ""
      let lineNumber = body["lineNumber"] as? Int ?? 0
      event = .annotationDraftSubmitted(annotationID: annotationID, commentID: commentID, body: draftBody, side: side, lineNumber: lineNumber)

    case "annotationDraftCancelled":
      let annotationID = body["annotationID"] as? String ?? ""
      let commentID = body["commentID"] as? String ?? ""
      let side = body["side"] as? String ?? ""
      let lineNumber = body["lineNumber"] as? Int ?? 0
      event = .annotationDraftCancelled(annotationID: annotationID, commentID: commentID, side: side, lineNumber: lineNumber)

    case "contentHeightChanged":
      let height = (body["height"] as? NSNumber)?.doubleValue ?? 0
      event = .contentHeightChanged(height: CGFloat(height))
